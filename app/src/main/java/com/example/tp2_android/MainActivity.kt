                ...
                Scaffold(
                    ...
                    NavHost(
                        ...
                        composable(Routes.DETAIL) { backStackEntry ->
                            val id =
                                backStackEntry.arguments?.getString("stationId")?.toInt() ?: 0
                            DetailScreen(
                                viewModel = viewModel,
                                stationIndex = id,
...
fun HomeScreen(
    ...
    viewModel: VeloViewModel = viewModel(), // injection automatique
    onStationClick: (V3Record) -> Unit = {} // callback navigation (Etape 4)
) {
    // --- OBSERVATION DES ETATS ---
    ...
    Column(modifier = modifier.fillMaxSize()) {
        ...
        Row(
            ...
            IconButton(onClick = { viewModel.loadData() }, enabled = !isRefreshing)
            {
                if (isRefreshing) {
                    CircularProgressIndicator(Modifier.size(24.dp), strokeWidth =
                        2.dp)
                } else {
                    Icon(Icons.Default.Refresh, "Rafraichir")
                }
            }
        }
            ...
            FilterChip(
                ...
                onClick = {
                    if (locationPermission.status.isGranted) {
                        if (sortByDistance) {
                            viewModel.toggleSortByDistance() // desactiver le tri
                        } else {
                            viewModel.fetchUserLocation(context) // activer + charger GPS
                        }
                    } else {
                        locationPermission.launchPermissionRequest() // demander la permission
                    }
                },
                label = { Text("Pres de moi") },
        ...
        Text(
            text = if (searchQuery.isEmpty()) messageStatus
            else "${stationsFiltrees.size} resultat(s)",
...
@Composable
fun StationCard(nom: String, velos: Int, places: Int, etat: String,
    ...
    Card(
        ...
        Row(
            ...
            Column(modifier = Modifier.weight(1f)) {
                ...
                if (estDeconnectee)
                    Text("HORS SERVICE", color = Color.Red,
                        style = MaterialTheme.typography.labelSmall)
                if (places == 0 && !estDeconnectee)
                    Text("Complet", color = Color(0xFFC0581A),
                        style = MaterialTheme.typography.labelSmall)
            }

            Column(modifier = Modifier.weight(1f)) {
                Text(nom, style = MaterialTheme.typography.titleMedium)
                // Affichage conditionnel de la distance
                if (distanceKm != null) {
                    Text(
                        text = if (distanceKm < 1f) "${(distanceKm * 1000).toInt()} m"
